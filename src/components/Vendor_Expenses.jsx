...
const Vendor_Expenses = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'Submitted', // Default status string
    notes: '', pdfFilePath: '',
  });
    ...
    <div className="min-h-screen bg-gray-900 p-6 text-gray-800">
      <div className="bg-white p-6 rounded-xl shadow-2xl w-full">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
          ...
          <div className="flex items-center gap-4">
            <span className="bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-bold uppercase">{currentUserRole} Mode</span>
            <button onClick={handleLogout} className="p-2 bg-red-100 text-red-600 rounded-full hover:bg-red-200"><LogOut size={20}/></button>
          </div>
        </div>
          ...
          <div className="mb-8 p-6 border-2 border-blue-200 rounded-xl bg-blue-50 shadow-md">
            ...
            <form onSubmit={handleSave} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <label className="block text-xs font-bold mb-1 uppercase">Vendor ID</label>
                ...
                <div className="lg:col-span-1"><label className="block text-xs font-bold mb-1 uppercase text-blue-700">PM Email</label>
                  <select id="pmEmail" className="w-full p-2 border rounded bg-white" value={formData.pmEmail} onChange={handleInputChange} required>
                    ...
                    <option value="Manas.Lalwani@revolvespl.com">Manas.Lalwani@revolvespl.com</option>
                    <option value="Nilesh.Peswani@revolvespl.com">Nilesh.Peswani@revolvespl.com</option>
                    <option value="abdulraees.shaikh@revolvespl.com">abdulraees.shaikh@revolvespl.com</option>
                    <option value="jony.rodrigues@revolvespl.com">jony.rodrigues@revolvespl.com</option>
                  </select>
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold mb-1 uppercase text-gray-500">Reason for Rejection / Internal Notes</label>
                <textarea id="notes" rows="1" className={`w-full p-2 border rounded ${currentUserRole === 'Admin' && formData.status === 'Rejected' ? 'bg-red-50 border-red-400' : 'bg-white'}`} value={formData.notes} onChange={handleInputChange} />
              </div>

              {/* FIXED ADMIN STATUS CONTROLS */}
              <div className="flex flex-wrap items-center justify-between p-4 bg-white rounded-lg border-2 border-dashed border-gray-200 gap-4">
                <div className="flex items-center gap-4">
                  <span className="text-xs font-black text-gray-500 uppercase">Set Record Status:</span>
                  <div className="flex gap-2">
                    <button 
                      type="button" 
                      onClick={() => setStatus('Submitted')}
                      className={`px-4 py-2 rounded-lg border-2 text-xs font-bold transition-all ${formData.status === 'Submitted' ? 'bg-blue-500 border-blue-500 text-white shadow-md' : 'bg-white text-gray-400'}`}
                    >
                      SUBMITTED
                    </button>
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg border-2 text-xs font-bold transition-all ${
                        formData.status === 'Approved' ? 'bg-green-600 border-green-600 text-white shadow-md' : 'bg-white text-gray-400 border-gray-200'
                      } ${currentUserRole !== 'Admin' ? 'opacity-20 grayscale cursor-not-allowed' : 'hover:border-green-600'}`}
                    >
                      APPROVE
                    </button>
                    <button 
                      ...
                      className={`px-4 py-2 rounded-lg border-2 text-xs font-bold transition-all ${
                        formData.status === 'Rejected' ? 'bg-red-600 border-red-600 text-white shadow-md' : 'bg-white text-gray-400 border-gray-200'
                      } ${currentUserRole !== 'Admin' ? 'opacity-20 grayscale cursor-not-allowed' : 'hover:border-red-600'}`}
                    >
                      REJECT
                    </button>
                  </div>
                ...
                <div className="flex gap-3">
                  <button type="submit" className="bg-blue-600 text-white px-8 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-blue-700 shadow-lg transition-all"><Save size={18}/> SAVE CHANGES</button>
                </div>
              </div>
            </form>
          </div>
          ...
          <table className="min-w-full divide-y divide-gray-200 text-sm">
            ...
            <tbody className="bg-white divide-y">
              {localEntries.map(entry => (
                <tr key={entry.id} onClick={() => { const n = new Set(); n.add(entry.id); setSelectedRows(n); }} className={`hover:bg-blue-50 transition-colors cursor-pointer ${selectedRows.has(entry.id) ? 'bg-blue-50' : ''}`}>
                  <td className="p-4 text-center"><input type="checkbox" checked={selectedRows.has(entry.id)} readOnly /></td>
                  <td className="px-6 py-3 font-medium">{entry.prime_key || entry.primeKey}</td>
                  <td className="px-6 py-3">
                    <span className={`font-bold uppercase text-xs px-2 py-1 rounded-full ${
                      (entry.status || (entry.is_approved ? 'Approved' : 'Submitted')) === 'Approved' ? 'bg-green-100 text-green-700' : 
                      (entry.status || (entry.is_approved ? 'Approved' : 'Submitted')) === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-blue-100 text-blue-700'
                    }`}>
                      {entry.status || (entry.is_approved ? 'Approved' : 'Submitted')}
                    </span>
                  </td>
                  <td className="px-6 py-3">{entry.vendor_name || entry.vendorName}</td>
                  <td className="px-6 py-3 font-bold">${parseFloat(entry.charge_amount || 0).toFixed(2)}</td>
                  <td className="px-6 py-3">{entry.pm_email || entry.pmEmail}</td>
                </tr>
              ))}
            </tbody>
          </table>
