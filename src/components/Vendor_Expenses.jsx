...
const Vendor_Expenses = ({ 
  ...
  const notifyPM = async (entry) => {
    setIsNotifying(true);
    const subject = entry.isApproved 
        ? `Action Required: Review Vendor Expense Record ${entry.primeKey}`
        : `Record Rejected: Vendor Expense Record ${entry.primeKey}`;
    
    const bodyContent = entry.isApproved
        ? `Hello,\n\nA new vendor expense record has been submitted and requires your attention.\n\nRecord Number: ${entry.primeKey}\n\nPlease log in to view and process this record.`
        : `Hello,\n\nVendor expense record ${entry.primeKey} has been REJECTED.\n\nReason for Rejection: ${entry.notes || 'No reason provided.'}\n\nPlease review the record and make the necessary corrections.`;
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/send-email`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          recipient: entry.pmEmail,
          subject: subject,
          bodyContent: bodyContent
        }),
      });
      if (response.ok) alert(`Notification sent to ${entry.pmEmail}`);
        ...
        <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-full text-gray-800">
              ...
              <div className="mb-8 p-6 border-2 border-blue-200 rounded-xl bg-blue-50 animate-in fade-in slide-in-from-top-4">
                ...
                <form onSubmit={(e) => handleSave(e, false)} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="lg:col-span-4 flex justify-end gap-3">
                    ...
                    <button type="button" onClick={(e) => handleSave(e, true)} className="bg-purple-600 text-white px-8 py-2 rounded-lg hover:bg-purple-700 transition flex items-center gap-2">
                      <Send size={18}/> Save and Notify
                    </button>
                  </div>
                </form>
              </div>
            ...
            <div className="flex gap-3 mb-6">
                ...
                <button onClick={() => {
                    const entryId = Array.from(selectedRows)[0];
                    const entry = localEntries.find(e => e.id === entryId);
                    if (entry) notifyPM(entry);
                }} disabled={selectedRows.size !== 1 || isNotifying} className="flex items-center gap-2 bg-purple-600 text-white px-5 py-2.5 rounded-lg disabled:opacity-50">
                    <Send size={20}/> {isNotifying ? 'Sending...' : 'Notify PM'}
                </button>
                <button onClick={startEdit} disabled={selectedRows.size !== 1} className="flex items-center gap-2 bg-gray-600 text-white px-5 py-2.5 rounded-lg disabled:opacity-50"><Pencil size={20}/> Edit</button>
                <button onClick={handleExport} disabled={selectedRows.size === 0 && !isExporting} className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-lg disabled:opacity-50">
                    <Download size={20}/> {isExporting ? 'Exporting...' : 'Export'}
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase">Notify PM</th>
                        </tr>
                    </thead>
                    <tbody className="bg-white divide-y">
                        {groupedEntries.length > 0 ? (
