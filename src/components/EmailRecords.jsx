...
const EmailRecords = ({ 
  ...
  const resetForm = () => {
    setFormData({
      ...
      contractShortName: '', sender: userName, pdfFilePath: ''
    });
    setIsAdding(false);
  ...
  const Row = ({ entry, isHistory = false }) => (
    <React.Fragment>
      ...
      <td className="px-6 py-3 whitespace-nowrap text-sm text-gray-700">{entry.contractShortName || 'N/A'}</td>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black p-4 sm:p-6 lg:p-8 text-gray-100 flex justify-center items-start">
      <div className="bg-white p-6 rounded-xl shadow-2xl w-full max-w-full text-gray-800">
        ...
        {/* Inline Form (Matches styling of Vendor form) */}
        {(isAdding || editingEntry) && (
          <div className="mb-8 p-6 border-2 border-blue-200 rounded-xl bg-blue-50 animate-in fade-in slide-in-from-top-4">
            ...
            <form onSubmit={handleSave} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-xs font-bold mb-1">SENT TO *</label>
              ...
              <div className="lg:col-span-4">
                <div className="flex justify-between items-center mb-1">
                    <label className="block text-xs font-bold uppercase">Body Preview</label>
                    <button type="button" onClick={copyToClipboard} className="text-xs flex items-center gap-1 text-blue-600 font-bold">
                        {copied ? <Check size={14}/> : <Copy size={14}/>} {copied ? 'Copied' : 'Copy Text'}
                    </button>
                </div>
                <textarea id="bodyContent" rows="4" className="w-full p-3 border rounded bg-white italic text-gray-600 text-sm" value={formData.bodyContent} onChange={handleInputChange} placeholder="Body will auto-populate based on type..."/>
              </div>

              <div className="lg:col-span-4 flex justify-end gap-3">
                <button type="button" onClick={resetForm} className="px-6 py-2 border rounded-lg text-gray-500 font-bold">Cancel</button>
                <button type="submit" className="bg-blue-600 text-white px-8 py-2 rounded-lg font-bold flex items-center gap-2">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center bg-gray-100 p-4 rounded-lg mb-6 gap-3 text-gray-700">
          <div className="flex items-center border rounded-lg bg-white flex-grow">
            <select value={searchColumn} onChange={(e) => setSearchColumn(e.target.value)} className="p-2 bg-transparent border-r text-sm outline-none">
        ...
        <div className="flex gap-3 mb-6">
          {!isAdding && !editingEntry && (
            <button onClick={() => setIsAdding(true)} className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-lg font-bold hover:bg-blue-700 transition">
        ...
        <div className="overflow-x-auto rounded-lg border border-gray-200">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              ...
              )) : <tr><td colSpan="8" className="p-10 text-center italic text-gray-400">No records found. Click "Add New" to get started.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
