...
import { Save, LogOut, Eye, EyeOff, Plus, Trash2, Briefcase, FileText, MapPin, Loader2, Utensils, List } from 'lucide-react';
...
const ExpenseManagementSystem = ({ 
  ...
  const [activeTab, setActiveTab] = useState('list'); // Default to 'list' for record view
  const [isPreviewOn, setIsPreviewOn] = useState(true);
  const [isSaving, setIsSaving] = useState(false);
  ...
  // ==========================================
  // 0. FETCH RECORDS LOGIC
  // ==========================================
  const fetchAllRecords = async () => {
    setIsLoadingRecords(true);
    const API_BASE = import.meta.env.VITE_API_BASE_URL;
    try {
      // Note: You may need a custom endpoint on your backend to merge these, 
      // or fetch them individually as shown here:
      const [travelRes, miscRes, mealsRes] = await Promise.all([
        fetch(`${API_BASE}/subk-travel/user/${currentUserId}`).then(res => res.json()).catch(() => []),
        fetch(`${API_BASE}/misc-expenses/user/${currentUserId}`).then(res => res.json()).catch(() => []),
        fetch(`${API_BASE}/business-meals/user/${currentUserId}`).then(res => res.json()).catch(() => [])
      ]);

      const combined = [
        ...travelRes.map(r => ({ ...r, type: 'Travel', displayDate: r.datePrepared, amount: r.totalAmount || 0 })),
        ...miscRes.map(r => ({ ...r, type: 'Misc', displayDate: r.items?.[0]?.date || 'N/A', amount: r.items?.reduce((s, i) => s + parseFloat(i.amount || 0), 0) })),
        ...mealsRes.map(r => ({ ...r, type: 'Meals', displayDate: r.items?.[0]?.date || 'N/A', amount: r.items?.reduce((s, i) => s + parseFloat(i.totalExpense || 0), 0) }))
      ];
      
      setRecords(combined.sort((a, b) => new Date(b.displayDate) - new Date(a.displayDate)));
    } catch (err) {
  ...
  useEffect(() => {
    if (activeTab === 'list') fetchAllRecords();
  }, [activeTab]);
    ...
    <div className="min-h-screen bg-slate-100 p-4 text-slate-800 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-4 bg-white p-3 rounded-xl shadow-sm border border-slate-200">
        <div className="flex items-center gap-4 w-full md:w-auto">
           <h1 className="text-lg font-black tracking-tight text-blue-900 hidden md:block ">Expense System</h1>
           <div className="flex bg-slate-100 p-1 rounded-lg w-full md:w-auto justify-center md:justify-start">
              <button onClick={() => setActiveTab('list')} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-md text-xs font-black tracking-wider transition-all ${activeTab === 'list' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><List size={14}/> Records</button>
              <button onClick={() => setActiveTab('travel')} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-md text-xs font-black tracking-wider transition-all ${activeTab === 'travel' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><Briefcase size={14}/> Travel</button>
              <button onClick={() => setActiveTab('misc')} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-md text-xs font-black tracking-wider transition-all ${activeTab === 'misc' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><FileText size={14}/> Misc</button>
              <button onClick={() => setActiveTab('meals')} className={`flex-1 md:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-md text-xs font-black tracking-wider transition-all ${activeTab === 'meals' ? 'bg-white text-blue-700 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}><Utensils size={14}/> Meals</button>
           </div>
      ...
      <div className="flex flex-col lg:flex-row gap-4 h-[calc(100vh-140px)]">
        ...
        <div className={`${(isPreviewOn && activeTab !== 'list') ? 'lg:w-1/3' : 'lg:w-full max-w-7xl mx-auto'} bg-white rounded-xl shadow-lg overflow-y-auto p-6 border-t-4 border-blue-600 transition-all duration-300`}>
          
          {/* RECORD VIEW TAB CONTENT */}
          {activeTab === 'list' && (
                ...
                <table className="w-full text-left text-xs border-collapse">
                  <thead>
                    <tr className="bg-slate-50 text-slate-400 font-black  tracking-widest border-b">
                      ...
                      <th className="p-4">Reference/Project</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {isLoadingRecords ? (
                      ...
                      <tr key={i} className="hover:bg-blue-50/50 transition-colors group">
                        <td className="p-4 font-bold text-slate-600">{rec.displayDate}</td>
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded text-[10px] font-black ${
                            rec.type === 'Travel' ? 'bg-blue-100 text-blue-700' : 
                            rec.type === 'Misc' ? 'bg-purple-100 text-purple-700' : 'bg-orange-100 text-orange-700'
                          }`}>
                            {rec.type}
                          </span>
                        </td>
          ...
          <form onSubmit={handleSave} className={`space-y-6 ${activeTab === 'list' ? 'hidden' : ''}`}>
        ...
        {/* --- RIGHT PANEL: PREVIEWS (Only shown for forms) --- */}
        {isPreviewOn && activeTab !== 'list' && (
          <div className="w-full lg:w-2/3 bg-white rounded-xl shadow-inner overflow-y-auto p-8 border border-slate-200">
