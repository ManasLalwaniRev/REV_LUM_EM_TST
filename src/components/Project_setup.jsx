...
const ProjectSetupForm = ({ 
  ...
  const handleDownloadTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Project_Template");
  ...
  const handleSubmit = (finalStatus) => {
    ...
    const newProject = { 
      ...
      id: Date.now(), 
      status: finalStatus,
      displayDate: formData.submissionDate 
    };
    setSubmittedProjects([...submittedProjects, newProject]);
    alert(`Project successfully ${finalStatus}!`);
    setActiveTab('list');
  };
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <div className="w-1/3 flex justify-center">
            <div className="flex bg-gray-100 p-1 rounded-lg border">
              <button onClick={() => setActiveTab('form')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold ${activeTab === 'form' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500'}`}><ClipboardList size={16} /> Setup Form</button>
              <button onClick={() => setActiveTab('list')} className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-bold ${activeTab === 'list' ? 'bg-white text-blue-700 shadow-sm' : 'text-gray-500'}`}><List size={16} /> New Projects</button>
            </div>
          </div>
          <div className="w-1/3 flex justify-end items-center gap-4">
            <div className="flex items-center gap-3 bg-gray-100 p-3 rounded-lg">
            ...
            <button onClick={handleLogout} className="p-3 bg-red-100 hover:bg-red-200 rounded-full text-red-600 transition-colors"><LogOut size={20} /></button>
          </div>
        </div>
        ...
        {activeTab === 'form' ? (
          <form className="space-y-10" onSubmit={(e) => e.preventDefault()}>
            ...
            <div className="flex gap-4 p-4 bg-gray-50 rounded-xl border-2 border-dashed border-gray-200">
              ...
              <label className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg font-bold text-sm cursor-pointer hover:bg-blue-700 transition-all shadow-md">
                <Upload size={16} /> Import Completed Excel
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="space-y-1">
                  ...
                  <input name="projectName" value={formData.projectName} onChange={handleInputChange} className="w-full p-2 border-b-2 border-gray-300 focus:border-blue-600 outline-none transition-colors" type="text" placeholder="e.g. Solar Phase II" required />
                </div>
                <div className="space-y-1">
                  <label className="text-xs font-bold text-gray-500 uppercase">Submitter Name</label>
            ...
            <section className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-4">
                  ...
                  <button type="button" className="flex items-center gap-2 bg-gray-50 hover:bg-gray-100 text-gray-700 px-6 py-2 rounded-lg border-2 border-dashed border-gray-300 transition-all text-sm font-semibold">
                ...
                <div className="overflow-x-auto border border-gray-200 rounded-xl">
                  <table className="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                      {[1, 2].map(row => (
                        <tr key={row} className="hover:bg-blue-50 transition-colors">
                          <td className="p-2"><input className="w-full p-2 border border-transparent rounded text-sm" type="text" placeholder="Name..." /></td>
                          <td className="p-2"><input className="w-full p-2 border border-transparent rounded text-sm" type="text" placeholder="PLC..." /></td>
                          <td className="p-2"><input className="w-full p-2 border border-transparent rounded text-sm" type="text" placeholder="CLIN..." /></td>
                          <td className="p-2"><input className="w-full p-2 border border-transparent rounded text-sm" type="text" placeholder="0.00" /></td>
            ...
            <section className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">11. Cost Ceiling / Burden / Fee Overrides</label>
                  <textarea name="billingOverrides" value={formData.billingOverrides} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" rows="3" placeholder="Detail any CLIN overrides..."></textarea>
                </div>
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">12. Billing Instructions</label>
                  <textarea name="billingInstructions" value={formData.billingInstructions} onChange={handleInputChange} className="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-sm" rows="3" placeholder="Enter format, emails, etc..."></textarea>
                </div>
              </div>
            </section>
            ...
            <div className="flex flex-wrap justify-end gap-4 pt-6 border-t border-gray-100">
                ...
                <button type="button" onClick={() => handleSubmit('submitted')} className="flex items-center gap-2 px-8 py-2.5 bg-blue-600 hover:bg-blue-700 text-white font-extrabold rounded-lg shadow-md transition-all text-sm">
                  <Save size={18} /> Submit Setup Form
                </button>
                {userName === 'Admin' && (
                  <button type="button" onClick={() => handleSubmit('approved')} className="flex items-center gap-2 px-8 py-2.5 bg-green-600 hover:bg-green-700 text-white font-extrabold rounded-lg shadow-md transition-all text-sm">
              ...
              <table className="min-w-full divide-y divide-gray-200 text-sm">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-left font-bold text-gray-600 uppercase text-xs">Status</th>
                ...
                <tbody className="divide-y divide-gray-200 bg-white">
                  {submittedProjects.map((project) => (
                    <tr key={project.id} className="hover:bg-blue-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-gray-600">{project.displayDate}</td>
                    </tr>
                  ))}
                  {submittedProjects.length === 0 && (
                    <tr><td colSpan="3" className="px-6 py-10 text-center text-gray-400 italic">No projects found. Use the setup form to record data.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
