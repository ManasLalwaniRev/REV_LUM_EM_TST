...
// --- SAMPLE DATA TO SHOW ALL SLA STATUSES ---
const MOCK_DATA = [
  {
    id: 1,
    primeKey: "V-101",
    ...
    infoReceivedDate: "2024-03-01",
    dateProcessed: "2024-03-01", // 0 days diff -> BEFORE DEADLINE (Green)
  ...
  },
  {
    ...
    primeKey: "E-202",
    moduleType: "email",
    subject: "Tax Invoice Query",
    recipient: "finance@company.com",
    task: "Review",
    emailDate: "2024-03-10",
    infoReceivedDate: "2024-03-10",
    dateProcessed: "2024-03-12", // 2 days diff -> ON DEADLINE (Yellow)
  ...
  },
  {
    ...
    primeKey: "B-303",
    moduleType: "bill",
    contractShortName: "Main Office",
    invoiceNo: "INV-9988",
    billingPeriod: "Feb 2024",
    amount: 5000.00,
    infoReceivedDate: "2024-03-01",
    dateProcessed: "2024-03-05", // 4 days diff -> DEADLINE CROSSED (Red)
    accountingProcessed: "T"
  },
  {
    id: 4,
    primeKey: "V-104",
    moduleType: "vendor",
    contractShortName: "Security",
    vendorName: "SafeGuard Inc",
    chargeAmount: 800.00,
    chargeDate: "2024-03-15",
    infoReceivedDate: "2024-03-15",
    dateProcessed: null, // No date -> AWAITING (Gray)
    accountingProcessed: "F"
  }
];
...
const SLA = ({ 
  dataEntries = MOCK_DATA, // Defaulted to mock data for demonstration
  isLoading = false, 
  error = null, 
  fetchEntries = () => {}, 
  userId = "USR-001", 
  userName = 'Accountant', 
  userAvatar = "https://ui-avatars.com/api/?name=Accountant&background=0D8ABC&color=fff", 
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-black p-4 sm:p-6 lg:p-8 text-gray-100 flex justify-center items-start">
      <div className="bg-white p-6 rounded-2xl shadow-2xl w-full max-w-full text-gray-800">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
          ...
          {Object.keys(editedData).length > 0 && (
            <button onClick={handleSaveAll} disabled={isSaving} className="bg-blue-600 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-blue-700">
              <Save size={20} /> {isSaving ? 'Saving...' : `Save ${Object.keys(editedData).length} Changes`}
            </button>
          )}
        </div>
        ...
        <div className="overflow-x-auto rounded-2xl border-2 border-gray-100">
          <table className="min-w-full divide-y divide-gray-200">
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {isLoading ? (
                <tr><td colSpan="10" className="p-12 text-center"><Loader className="animate-spin mx-auto text-blue-600" size={32}/></td></tr>
              ) : groupedEntries.length === 0 ? (
                ...
                // Priority: Edited State -> CamelCase -> Snake_case -> null
                const curInfoDate = editedData[entryId]?.infoReceivedDate 
                                 ?? latest.infoReceivedDate 
                                 ?? latest.info_received_date 
                                 ?? null;
                ...
                return (
                  <tr key={latest.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    {tabConfig[activeTab].keys.map(key => (
                      <td key={key} className="px-4 py-4 text-xs font-bold text-gray-600">
                        {key.includes('Amount') 
                          ? `$${parseFloat(latest[key] || 0).toLocaleString(undefined, {minimumFractionDigits:2})}` 
                          : latest[key] || '---'}
                      </td>
                    ))}
                    {/* Visual SLA Column */}
                    ...
                    <td className="px-4 py-4">
                      <input 
                        ...
                        value={editedData[latest.id]?.infoReceivedDate ?? formatDateForInput(latest.infoReceivedDate || latest.info_received_date)}
                        onChange={e => handleEditChange(latest.id, 'infoReceivedDate', e.target.value)}
                        className="text-xs font-bold border rounded p-1 focus:ring-2 focus:ring-blue-500 outline-none"
                      />
                    </td>
                    <td className="px-4 py-4">
                      <input 
